% SFT + KTO iterated training loop — poster figure
% Compile: tectonic loop_sft_kto.tex
...
\begin{scope}[shift={(0,-3.2)}]
  \node[chip] (s3a) at (-2.3,0) {Generate\\Dialogues};
  \node[chip] (s3b) at (0,0)    {Critic\\Evaluation};
  \node[chip] (s3c) at (2.3,0)  {Score All\\Rounds};
  \draw[chiparrow] (s3a) -- (s3b);
  \draw[chiparrow] (s3b) -- (s3c);
  \draw[loopback] (s3c.south) to[bend left=22] (s3a.south);
  \node[font=\bfseries, text=navy] (s3t) at (0,1.0) {3 · ROLEPLAY};
  \node[foot] (s3f) at (0,-1.15) {CTRS-R total per dialogue (0--33)};
  \begin{scope}[on background layer]
...
\begin{scope}[shift={(-6.1,0)}]
  \node[font=\bfseries, text=textred] (s4t) at (0,0.75) {4 · KTO TRAINING};
  \node[labelchip, fill=chipgreenbg, draw=chipgreenbd, thick] (s4a) at (-1.15,-0.1)
        {\textcolor{chipgreenfg}{\footnotesize$\blacktriangle$ \textbf{desirable}}\\
         \textcolor{chipgreenfg}{\tiny above $+0.5\sigma$}};
  \node[labelchip, fill=chipredbg, draw=borderred, thick] (s4b) at (1.15,-0.1)
        {\textcolor{textred}{\footnotesize$\blacktriangledown$ \textbf{undesirable}}\\
         \textcolor{textred}{\tiny below $-0.5\sigma$}};
  \node[font=\scriptsize, text=subred] (s4f) at (0,-1.05)
        {session score $\rightarrow$ per-turn labels};
  \begin{scope}[on background layer]
